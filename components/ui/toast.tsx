...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed top-[calc(1rem+env(safe-area-inset-top,0px))] right-[calc(1rem+env(safe-area-inset-right,0px))] left-[calc(1rem+env(safe-area-inset-left,0px))] z-[100] w-auto max-w-sm outline-none sm:right-4 sm:left-auto sm:w-full",
        className
      )}
      {...props}
    />
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "success") {
    icon = (
      <CircleCheckIcon className="text-success dark:text-success" aria-hidden="true" />
    )
  }

  if (type === "info") {
    icon = (
      <InfoIcon className="text-primary" aria-hidden="true" />
    )
  }

  if (type === "warning") {
    icon = (
      <TriangleAlertIcon className="text-warning dark:text-warning" aria-hidden="true" />
    )
  }
